        ...
        <el-card>
            ...
            <el-row class="cat_opt">
                <el-col>
                    <span>选择商品分类:</span>
                    <!-- 选择商品分类的级联选择框 -->
                    <el-cascader v-model="selectCateKeys" :options="this.cateList" :props="catProps" @change="handleChange" ></el-cascader>
                </el-col>
            </el-row>
            <!-- tabs标签页 -->
                    ...
                    <el-table :data="manyTableData" border stripe>
                        ...
                        <el-table-column type="expand">
                            <template slot-scope="scope">
                                ...
                                :key="i"
                                closable
                                v-show="scope.row.attr_vals[0]"
                                @close="handleClose(i,scope.row)">
                                {{item}}
                                </el-tag>

                                <!-- 如果该分组下没有数据则提示没有 -->
                                <!-- <p v-show="!scope.row.attr_vals[0]">暂无数据</p> -->
    ...
    methods: {
        ...
        addParmas(){
            this.$refs.addFromRef.validate(async valid=>{
                ...
                if(res.meta.status!==201){
                    return this.$message.error('发送数据失败')
                }
                this.$message.success('发送数据成功')
        ...
        editParmas(){
            this.$refs.editFromRef.validate(async valid=>{
                ...
                // console.log(res);
                if(res.meta.status!==200){
                    return this.$message.error('推送数据失败')
                }
